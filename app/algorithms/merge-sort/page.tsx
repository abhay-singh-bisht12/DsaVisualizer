      ...
      <Card className="p-6 mb-6">
        ...
        <div className="mb-6 p-4 rounded-lg border-2 border-blue-200 dark:border-blue-800 bg-white/50 dark:bg-slate-900/50">
          ...
          <p className="text-muted-foreground leading-relaxed">
            <strong>Merge Sort</strong> is a divide-and-conquer, comparison-based sorting algorithm that splits the array into two halves recursively until each subarray has at most one element, then merges the sorted halves back together by repeatedly comparing the smallest remaining elements from each half and appending the smaller one to the result. It is stable and has guaranteed O(n log n) time complexity but uses O(n) extra space for the merge step.
          </p>
        </div>
        ...
        <ol className="list-decimal pl-6 space-y-4 text-muted-foreground mb-4">
          <li>
            <strong className="text-foreground">Divide.</strong> If the array has 0 or 1 element, it is already sorted; return. Otherwise, find <code className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">mid</code> and split into <code className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">left = array[0..mid−1]</code> and <code className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">right = array[mid..n−1]</code>.
          </li>
          <li>
            <strong className="text-foreground">Conquer.</strong> Recursively sort <code className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">left</code> and <code className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">right</code> using merge sort. (Each will eventually be split until base case.)
          </li>
          <li>
            <strong className="text-foreground">Merge.</strong> Merge the two sorted halves into one sorted array: compare the first unused element of each half, append the smaller to the result, and repeat until both halves are used. If one half is exhausted, append the rest of the other.
          </li>
          <li>
            <strong className="text-foreground">Return.</strong> The merged array is the sorted result for the current range.
          </li>
        </ol>
        ...
        <div className="mt-6 flex justify-end">
          <CheatSheetDownload
            fileName="merge-sort-cheatsheet.txt"
            title="Merge Sort"
            definition="Merge Sort is a stable divide-and-conquer sorting algorithm that recursively splits the array into halves until single elements remain, then merges the sorted halves back together in order."
            steps={[
              "If the array has 0 or 1 element, it is already sorted (base case).",
              "Divide the array into two halves (left and right).",
              "Recursively sort the left half and the right half using merge sort.",
              "Merge the two sorted halves by repeatedly taking the smaller front element from either half.",
              "Return the merged result as the sorted array for that range.",
            ]}
          />
        </div>
      </Card>
